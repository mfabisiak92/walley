...
- **Creation wizard**, in order: Income → Income-related expenses → Fixed costs → Other costs → Savings → Investments → Summary. You can move back and forth freely until you hit Create.
  ...
  - Other costs items can *optionally* be linked to a **Checking, Cash, or Saving account** (not Investment) — for costs paid straight out of savings (e.g. an emergency withdrawal) or another account. If the linked account is a Saving account, the amount can't exceed that account's current balance.
...
- **Editing an item's linked account**: for Income, Income-related expenses, and Other costs items, the long-press edit dialog also lets you reassign which account the item is linked to (or clear it, for the optional Other costs case) — same account-type restrictions and Saving-balance cap as when the item was created. If any of the item has already been paid, the already-applied balance effect moves from the old account to the new one. Savings and Investments items keep their account fixed to what was chosen when the item was created, since the account *is* the item there.
