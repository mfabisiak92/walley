...
- **Draw from linked accounts (on/off, per category)**: four independent switches, on by default — one each for **Income & income-related expenses**, **Fixed & other costs**, **Savings**, and **Investments**. When a category's switch is off, none of the account side effects above happen for items in that category (money stops moving for just that group; the rest of the budget is unaffected) — items are still tracked as paid/unpaid regardless. All four are set on the wizard's first step (right alongside choosing the month) so they're already in place for Drafts. Once a budget exists, a gear icon on its detail screen's top bar opens a dedicated **Budget settings** screen to change any of the four later, for as long as the budget is Active. They're independent per budget; flipping one never retroactively reverses or reapplies balance changes from payments made before the switch — it only governs actions from that point on.
