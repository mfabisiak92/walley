...
- **Item icons**: every item can have a small colored icon, picked from a curated set: Salary/Dividends/Interest for Income (matching its category), Saving/Investment (assigned automatically for Savings/Investments items, no picker needed), and 38 general expense icons — Tax, Car, Fuel, Subscription, Groceries, Clothes, Eating out, Entertainment, Transportation, Vacations, Trip, Gift, Insurance, Currency exchange, Electronics, Phone, Bills, Rent, Piggy bank, Electricity, Internet, TV, Music, Barber, Games, Sport, Books, Repair, Health, Medicine, Pet, Movie, Kids, Retirement, Parking, Ship, Shipment, Doctor — offered for Income-related expenses, Fixed costs, and Other costs. Picked when adding/editing an item (both in the creation wizard and via an existing item's long-press edit dialog). Items created before this feature got an icon backfilled automatically where possible, based on their income category, section, or a keyword match against their name.
...
- **Draw from linked accounts (on/off)**: a per-budget switch, on by default. When off, none of the account side effects above happen for that budget — items are still tracked as paid/unpaid, but no money moves in any account (income deposits, Savings/Investments contributions, and Fixed/Other costs withdrawals are all skipped). Set on the wizard's first step (right alongside choosing the month) so it's already in place for Drafts, and stays editable afterward from the Summary tab for as long as the budget is Active. It's independent per budget; flipping it never retroactively reverses or reapplies balance changes from payments made before the switch — it only governs actions from that point on.
...
- **Draw from linked account (on/off)**: a switch on the wizard's first step (right alongside picking the date range and account), on by default, and independent from every other budget (Monthly or Ad-hoc). Stays editable afterward from the detail screen. When off, paying items just tracks progress without withdrawing from the linked account. Same "no retroactive effect" rule as Monthly — flipping it only changes what happens to future pay/edit/delete actions.
